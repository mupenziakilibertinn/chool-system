...
export default function TeacherPortal() {
  ...
  const targetMark = selectedClass === "P6" ? 100 : 50; 
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (user) => {
      if (user?.email) {
        ...
        setUserEmail(email);
        if (email === "mupenziakilibertinn@gmail.com") {
          setMyClasses(["P1", "P2", "P3", "P4", "P5", "P6"]);
          setMySubjects(["Mathematics", "Kinyarwanda", "English", "Science & Elem. Tech (SET)", "SRE / Religion", "French"]);
          setSelectedClass("P5"); setSubject("Mathematics");
          return;
        }
  ...
  const handleAutoSave = (studentId: string, field: string, raw: string) => {
    if (!raw) return;
    const converted = Math.round((Number(raw) / testOutOf) * targetMark);
    setStatus("Saving...");
    setDoc(doc(db, "students", studentId, "marks", subject), { [field]: converted }, { merge: true }).then(() => setStatus("Saved"));
  };

  if (!userEmail) return <div className="p-20 text-center font-black">Loading...</div>;

  return (
    <div className="min-h-screen bg-white">
      <div className="bg-blue-900 p-6 text-white sticky top-0 z-50 flex justify-between items-center shadow-lg">
         <h1 className="text-lg font-black uppercase italic">NGS Teacher Portal</h1>
         <div className="flex gap-3">
            <div className="bg-blue-800 p-1 rounded-lg flex items-center gap-2 border border-blue-400">
               <span className="text-[8px] font-black uppercase px-2">Paper Max:</span>
      ...
      <table className="w-full text-left border-collapse">
        <thead className="bg-gray-100 border-b text-[10px] font-black uppercase text-blue-900">
