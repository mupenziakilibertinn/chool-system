...
// Updated list order to perfectly match the sequence in your picture
...
function ReportCardsEngine() {
  ...
  useEffect(() => {
    const fetchClassReports = async () => {
      ...
      try {
        ...
        // 3. Fetch All Student Marks Matrices 
        let marksMatrix: any = {};
        ...
        const studentsWithScores = classFiltered.map((student) => {
          ...
          // B. Process Co-Curricular Matrix Integration (From Class Teacher Inputs)
          coCuriList.forEach((cc) => {
            const ccData = studentMarks[cc.id] || {};
            const t1 = ccData[`${selectedTerm}_t1`] ?? "-";
            const m1 = ccData[`${selectedTerm}_m1`] ?? "-";
            const t2 = ccData[`${selectedTerm}_t2`] ?? "-";
            const m2 = ccData[`${selectedTerm}_m2`] ?? "-";
        ...
        // 5. Establish Rank Positioning via Summation Calculations
  ...
  const getAutomaticComment = (percentage: number, max: number) => {
    ...
    if (percentage >= 85) return "An exceptional performance this term! Highly disciplined, consistent, and exemplary work across all course and co-curricular pathways. Keep up this brilliant standard.";
    if (percentage >= 70) return "A very strong and commendable performance. Shows steady focus, capability, and balanced talent development. Keep pushing for higher grades.";
    ...
    <div className="min-h-screen bg-gray-100 font-sans text-xs pb-20">
      <style dangerouslySetInnerHTML={{__html: `
        @media print {
          ...
          .print-teacher-line { font-size: 12px !important; width: 240px !important; height: 32px !important; border: none !important; }
              ...
              >
                ...
                <div className="flex flex-col justify-start space-y-3 flex-grow">
                  ...
                  <table className="w-full text-center border-collapse border-4 border-black text-xs font-black print-table">
                    <thead className="bg-gray-100 border-b-4 border-black uppercase text-[9px] tracking-wider">
                      <tr>
                        ...
                        {reportMode === "summation" && (
                          <>
                            <th className="p-2 border-r-2 border-black">T1 (/50)</th>
                            <th className="p-2 border-r-2 border-black">M1 (/50)</th>
                            <th className="p-2 border-r-2 border-black">T2 (/50)</th>
                            <th className="p-2 border-r-4 border-black">M2 (/50)</th>
                          </>
                        )}
                    ...
                    <tbody>
                      {/* Section A: Academic Subjects in explicit visual order */}
                      ...
                      <tr className="bg-gray-100/50 text-[10px] uppercase border-t-2 border-b-2 border-black">
                        <td colSpan={reportMode === "summation" ? 6 : 4} className="p-1 text-left px-2 font-black tracking-wider text-gray-500">
                          Co-Curricular Observations
                        </td>
                      </tr>
                      ...
                      {coCuriList.map((cc) => {
                        ...
                        const hasCCMarks = t1 !== "-" || m1 !== "-" || t2 !== "-" || m2 !== "-";

                        return (
                          <tr key={cc.id} className="border-b-2 border-black text-gray-900 text-xs font-black italic">
                            <td className="p-2 py-2 border-r-4 border-black text-left font-black uppercase text-emerald-950">{cc.name}</td>
                            {reportMode === "mid1" && (
                              <>
                                <td className="p-2 border-r-2 border-black text-gray-500">{t1}</td>
                                <td className="p-2 border-r-4 border-black text-gray-500">{m1}</td>
                              </>
                            )}
                            {reportMode === "mid2" && (
                              <>
                                <td className="p-2 border-r-2 border-black text-gray-500">{t2}</td>
                                <td className="p-2 border-r-4 border-black text-gray-500">{m2}</td>
                              </>
                            )}
                            {reportMode === "summation" && (
                              <>
                                <td className="p-2 border-r-2 border-black text-gray-500">{t1}</td>
                                <td className="p-2 border-r-2 border-black text-gray-500">{m1}</td>
                                <td className="p-2 border-r-2 border-black text-gray-500">{t2}</td>
                                <td className="p-2 border-r-4 border-black text-gray-500">{m2}</td>
                              </>
                            )}
                            <td className="p-2 font-black text-emerald-800">
                              {hasCCMarks ? `${ccTotal} / 10` : "-"}
                            </td>
                          </tr>
                        );
                      })}

                      {/* Section C: Combined Summation Standing */}
                ...
                {/* Signatures & Stamps Footer Layout */}
